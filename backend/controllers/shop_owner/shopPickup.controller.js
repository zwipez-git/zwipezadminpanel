...
const getDeliveryIdFromToken = (req) => {
  ...
  if (!deliveryId) {
    return {
      error:
        "Delivery partner not found in token",
      code: 400,
    };
  }
...
// ✅ CREATE OTP
export const createOrRefreshPickupOtp =
  async (req, res) => {
    try {
      const auth =
        getShopIdFromToken(req);
      ...
      if (
        ...
        return res.status(400).json({
          status: 0,
          message:
            "Pickup OTP can be created only for ready orders",
        });
      }
      ...
      return res.json({
        status: 1,
        message:
          `Pickup OTP created with ${PICKUP_OTP_VALIDITY_HOURS} hour validity`,
        data: otpResult.rows[0],
      });
    } catch (err) {
      console.error(
        "Create pickup OTP error:",
        err
      );
...
// ✅ GET OTP STATUS
export const getPickupOtpStatus =
  async (req, res) => {
    try {
      const auth =
        getShopIdFromToken(req);
      ...
      return res.json({
        status: 1,
        message:
          "Pickup OTP status fetched",
        data: result.rows[0],
      });
    } catch (err) {
      console.error(
        "Get pickup OTP status error:",
        err
      );
...
// ✅ VERIFY OTP
export const verifyPickupOtpAndMarkPickedUp =
  async (req, res) => {
    const client =
      await pool.connect();
    ...
    try {
      ...
      if (
        ...
        return res.status(400).json({
          status: 0,
          message:
            "Only ready orders can be marked picked up",
        });
      }
      ...
      if (!otpResult.rows.length) {
        ...
        return res.status(400).json({
          status: 0,
          message:
            "Pickup OTP not generated for this order",
        });
      }
      ...
      if (
        ...
        return res.status(400).json({
          status: 0,
          message:
            "Pickup OTP expired. Generate a new OTP.",
        });
      }
